...
make_title <- function(df){
    ...
    if(any(grepl("I2", nms))){
        nms[grepl("I2", nms)] <- ifelse(grep("I2", nms) == 1, "I^2~\"", "\"~I^2~\"")
    }
...
for(x in list_seq){ # loop over summary (eg. dist)
    ...
    for(y in seq_len(nrow(grid_others))){ # loop over all combinations of other parameters (e.g. bias, large, he)
        ...
        lapply(current_levels, function(z){
            title <- img_data %>% 
                filter(!!sym(current) == z) %>% 
                select(-k, -method, -measure, -value, -all_of(current), -sampleSize) %>%
                distinct() %>%
                bind_cols(!!current := z) %>%
                select(order(colnames(.))) %>%
                make_title()
...
for(x in list_seq){ # loop over summary (eg. dist)
    ...
    for(y in seq_len(nrow(grid_others))){ # loop over all combinations of other parameters (e.g. bias, large, he)
        ...
        lapply(current_levels, function(z){
            title <- img_data %>% 
                filter(!!sym(current) == z) %>% 
                select(-method, -measure, -value, -all_of(current), -sampleSize) %>%
                distinct() %>%
                bind_cols(!!current := z) %>%
                select(order(colnames(.))) %>%
                make_title()
            img_data %>% 
                ...
                ylim(c(0, 1)) +
                facet_wrap(~method) +
                labs(x = "# intervals", y = "relative frequency",
                     title = eval(title))
        }) %>% 
            wrap_plots(guides = "collect") &
            theme(legend.position = "bottom",
