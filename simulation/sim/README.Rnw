...
\node (ci) [most, below of=tau, font=\footnotesize] {3. Calculate CIs with all
  methods for all $\tau^2$};
\node (meas) [most, below of=ci, font=\footnotesize] {4. Calculate
...
generated inside different for loops are independent and reproducible. This
setup has, however, the drawback that without manual error handling, the
...
simulated meta-analysis data set into the CIs. The idea here is to
...
%
% \begin{itemize}
%   \item \texttt{calc\_coverage\_effects()}
%   \item \texttt{calc\_coverage\_effects\_min1()}
%   \item \texttt{calc\_coverage\_effects\_all()}
%   \item \texttt{calc\_coverage\_prediction()}
%   \item \texttt{calc\_width()}
%   \item \texttt{calc\_coverage\_true()}
%   \item \texttt{calc\_gamma\_min()}
%   \item \texttt{calc\_score()}
%   \item \texttt{calc\_n()}
%   \item \texttt{calc\_sq\_diff()}
%   \item \texttt{calc\_estimate()}
%   \item \texttt{get\_measures()}
%   \item \texttt{CI2measures()}
%   \item \texttt{calc\_measures()}
% \end{itemize}
%
% \todo{describe arguments, describe structure}
%
% \subsection{measures2summary.R}
%
% \begin{itemize}
